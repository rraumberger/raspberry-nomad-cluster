#!/bin/bash
COMMUNICATION_SERVER=$(/opt/dynatrace/oneagent/agent/tools/oneagentctl --get-server | cut -d '*' -f 2 | cut -d ';' -f 1)
...
curl --insecure -L -X POST "${COMMUNICATION_SERVER}/e/${TENANT_ID}/api/v2/metrics/ingest" \
